...
export const ERROR_RULE_CATALOG: Array<{
  ...
  { id: 'health', label: 'Score équipement < 70', trigger: 'Suspendu : score non calculable tant que les règles métier ne sont pas validées', audience: 'Facility Manager et Administration', defaultChannels: ['in-app', 'email'] },
    ...
    <section className="error-rule-list" aria-labelledby="error-rules-title">
      ...
      <ul>
        {ERROR_RULE_CATALOG.map((item) => {
          ...
          const scoreRuleSuspended = item.id === 'health';
          return (
            <li key={item.id} className={rule.active ? '' : 'is-off'}>
              ...
              <div className="error-rule-controls">
                <Switch on={scoreRuleSuspended ? false : rule.active} label={scoreRuleSuspended ? `${item.label} indisponible` : `${rule.active ? 'Désactiver' : 'Activer'} ${item.label}`} disabled={!canEdit || scoreRuleSuspended} onToggle={() => canEdit && !scoreRuleSuspended && update(item.id, { active: !rule.active })} />
                <label>
                  <input
                    type="checkbox"
                    checked={rule.channels.includes('email')}
                    disabled={!canEdit || !rule.active || scoreRuleSuspended}
                    onChange={() => {

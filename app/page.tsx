...
function Detail({ anomaly, decisionAmount, persistenceMode, persistenceEnabled, offlineSync, onBack, onStatus, onProof, onVerify, readOnly = false, canVerify = false, busy = false }: { anomaly:Anomaly; decisionAmount:number|null; persistenceMode:'demo'|'server'; persistenceEnabled:boolean; offlineSync:ReturnType<typeof useOfflineSync>; onBack:()=>void; onStatus:(s:Status)=>void; onProof:(file:File)=>Promise<SyncStatusState>; onVerify:()=>void; readOnly?:boolean; canVerify?:boolean; busy?:boolean }) {
...
function Report({ persona, onNavigate, persistenceEnabled, offlineSync, flash }: {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void saveDraft<RoundDraft>(draftId, { step, pressure, tankLevel, observation, checks, quickTitle, quickPriority, quickZone, quickControlType, confirmed });
    }, 450);
    return () => window.clearTimeout(timer);
  }, [checks, confirmed, draftId, draftReady, observation, persistenceEnabled, pressure, quickControlType, quickPriority, quickTitle, quickZone, saveDraft, step, submitted, tankLevel]);
  ...
  if (!surpresseurAccess) return <>
    ...
    {submitted && <div className="prototype-success" role="status"><span>✓</span><div><b>{persistenceEnabled ? 'Constat placé dans la file de synchronisation' : 'Simulation de constat terminée'}</b><small>{persistenceEnabled ? 'L’identifiant unique empêche toute création en double lors d’une reprise.' : 'Aucune donnée n’a été enregistrée ou transmise.'}</small></div><button onClick={() => onNavigate('workspace')}>Retour à mon espace</button></div>}
  </>;

  return <>
    <section className="surpresseur-hero">
      <div className="surpresseur-identity"><span className="surpresseur-monogram">WI</span><div><p className="design-kicker">MODULE PILOTE · SURPRESSEUR</p><h2>Ronde Surpresseur · WILO-01</h2><p>Sous-sol · Local surpresseur · Fréquence quotidienne</p></div></div>
      <div className="surpresseur-health"><span>Score santé</span><strong>78<small>/100</small></strong><p className="kpi-status is-watch">Surveillance</p></div>
    </section>
    <OfflineSyncStatus enabled={persistenceEnabled} online={offlineSync.online} running={offlineSync.running} counts={offlineSync.counts} latestIssue={offlineSync.latestIssue} onRetry={() => void offlineSync.retryFailed().then(() => offlineSync.synchronize())} />

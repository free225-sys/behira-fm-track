...
type FieldRequest = {
  ...
  subject: string;
  note: string;
  status: 'À traiter par Faustin' | 'Complément transmis' | 'Transmise à Direction';
};
...
const seedAnomalies: Anomaly[] = [
  { id:'ANO-0241', asset:'RIA-01', title:'Pression réseau incendie instable', location:'Sous-sol · Local incendie', priority:'Critique', status:'À qualifier', reported:'24 août · 07:36', due:'Aujourd’hui · 12:00', owner:'Non affectée', delayed:false, proof:false, description:'Variations de pression constatées pendant le test matinal. Le manomètre oscille entre 5,8 et 7,2 bars sans sollicitation du réseau.' },
  { id:'ANO-0238', asset:'ASC-A2', title:'Arrêts intermittents au niveau R+7', location:'Tour A · Ascenseur 2', priority:'Haute', status:'Affectée', reported:'23 août · 08:15', due:'23 août · 18:00', owner:'Kone Elevators', delayed:true, proof:false, description:'Deux arrêts non programmés signalés au niveau R+7. Redémarrage automatique après environ trente secondes.' },
  { id:'ANO-0234', asset:'WILO-01', title:'Fuite légère au collecteur', location:'Sous-sol · Local surpresseur', priority:'Moyenne', status:'En intervention', reported:'21 août · 16:42', due:'22 août · 15:00', owner:'Hydro Services', delayed:true, proof:false, description:'Suintement visible au raccord du collecteur principal. Bac de rétention en place, sans impact sur la distribution.' },
  { id:'ANO-0231', asset:'GE-01', title:'Batterie de démarrage sous tension nominale', location:'RDC · Local groupe', priority:'Critique', status:'En validation', reported:'20 août · 11:20', due:'21 août · 10:00', owner:'PowerTech', delayed:true, proof:true, description:'La batterie mesurée à 11,6 V a été remplacée. Le test de démarrage est concluant, preuve en attente de validation FM.' },
  { id:'ANO-0229', asset:'IRR-01', title:'Électrovanne zone jardin bloquée', location:'Extérieur · Jardin nord', priority:'Faible', status:'Clôturée', reported:'19 août · 09:05', due:'20 août · 17:00', owner:'GreenCare', delayed:false, proof:true, description:'Électrovanne nettoyée et remise en service. Cycle d’arrosage contrôlé sur vingt minutes.' },
  { id:'ANO-0226', asset:'ASC-A1', title:'Éclairage cabine défaillant', location:'Tour A · Ascenseur 1', priority:'Moyenne', status:'Clôturée', reported:'18 août · 14:30', due:'19 août · 12:00', owner:'Kone Elevators', delayed:false, proof:true, description:'Bloc LED remplacé et essai d’éclairage de secours réalisé.' },
  { id:'ANO-0222', asset:'RND-LET', title:'Porte coupe-feu maintenue ouverte', location:'R+4 · Circulation Est', priority:'Haute', status:'À qualifier', reported:'24 août · 06:58', due:'Aujourd’hui · 14:00', owner:'Non affectée', delayed:false, proof:false, description:'Le ferme-porte ne ramène plus complètement le vantail. Zone balisée pendant la ronde.' },
  { id:'ANO-0218', asset:'GE-01', title:'Niveau carburant inférieur au seuil', location:'RDC · Local groupe', priority:'Moyenne', status:'Affectée', reported:'17 août · 10:10', due:'Aujourd’hui · 17:00', owner:'PowerTech', delayed:false, proof:false, description:'Niveau à 28 %, demande de réapprovisionnement transmise au prestataire.' },
];
...
const DEMO_PASSWORD = 'Behira-Demo-2026!';
const SESSION_KEY = 'behira_demo_session_v1';
const demoAccounts: DemoAccount[] = [
  { personaId:'frederic', email:'direction@demo.behira.invalid', password:DEMO_PASSWORD, destination:'Espace Direction' },
  { personaId:'faustin', email:'facility.manager@demo.behira.invalid', password:DEMO_PASSWORD, destination:'Facility Manager' },
  { personaId:'evariste', email:'electricite@demo.behira.invalid', password:DEMO_PASSWORD, destination:'Espace Évariste' },
  { personaId:'sylvain', email:'eau.incendie@demo.behira.invalid', password:DEMO_PASSWORD, destination:'Espace Sylvain' },
  { personaId:'laetitia', email:'rondes@demo.behira.invalid', password:DEMO_PASSWORD, destination:'Espace Laetitia' },
];

const allowedViewsByPersona: Record<PersonaId, View[]> = {
  faustin:['workspace','dashboard','registry','manager','report'],
  frederic:['workspace','dashboard','registry'],
  evariste:['workspace','report'],
  sylvain:['workspace','report'],
  laetitia:['workspace','report'],
};
...
const seedEscalations: Escalation[] = [
  { id:'DEC-018', anomaly:'ANO-0241', asset:'RIA-01', title:'Valider le maintien en service sous surveillance', kind:'Risque', due:'Aujourd’hui · 10:30', risk:'Sécurité incendie · impact critique', recommendation:'Maintenir sous surveillance 24 h avec contrôle SECURISYS.', state:'À décider' },
  { id:'DEC-017', anomaly:'ANO-0231', asset:'GE-01', title:'Remplacement préventif du banc batteries', kind:'Coût', amount:3800000, due:'Aujourd’hui · 15:00', risk:'Continuité électrique · CAPEX', recommendation:'Remplacer maintenant plutôt que multiplier les maintenances curatives.', state:'À décider' },
  { id:'DEC-016', anomaly:'ANO-0238', asset:'ASC-A2', title:'Arbitrer intervention interne ou ATA', kind:'Arbitrage', amount:1650000, due:'En retard · 23 août', risk:'Perte de redondance ascenseurs', recommendation:'Confier le diagnostic à ATA et conserver Évariste en appui.', state:'À décider' },
  { id:'DEC-015', anomaly:'ANO-0234', asset:'WILO-01', title:'Validation finale de clôture après récidive', kind:'Clôture sensible', amount:2900000, due:'Aujourd’hui · 17:00', risk:'Deux réarmements provisoires · OPEX élevé', recommendation:'Refuser la clôture tant que la cause racine et le PV ne sont pas fournis.', state:'À décider' },
  { id:'DEC-014', anomaly:'ANO-0226', asset:'ASC-A1', title:'Clôture finale après risque usager', kind:'Clôture sensible', due:'Décidée le 22 août', risk:'Preuves ATA conformes', recommendation:'Clôturer avec suivi de récurrence à 30 jours.', state:'Approuvée', motive:'Preuves complètes et essai de sécurité concluant.' },
];
...
const fallbackEquipment: EquipmentItem[] = [
  { code:'GE-01', label:'Groupe électrogène', health:86, state:'Surveillance' },
  { code:'WILO-01', label:'Surpresseur', health:78, state:'Intervention' },
  { code:'RIA-01', label:'Pompe incendie', health:61, state:'Critique' },
  { code:'ASC-A1/A2', label:'Ascenseurs', health:84, state:'Surveillance' },
  { code:'IRR-01', label:'Irrigation', health:98, state:'Sain' },
  { code:'RND-LET', label:'Rondes & constats', health:93, state:'Sain' },
];
...
const fallbackVendors: OperationalVendor[] = [
  { code:'DMC', label:'DM COMPANY' },
  { code:'ATA-CI', label:'ATA CI / ALBEDO' },
  { code:'SECURISYS', label:'SECURISYS' },
  { code:'ALTA-VENTURE', label:'ALTA VENTURE' },
];
...
function AuthExperience({ onAuthenticate, onDemoAuthenticate, onForgot, onReset, supabaseMode, allowDemoFallback, environmentLabel }: {
  ...
  const submitLogin = async (event:FormEvent) => {
    ...
    try {
      await onAuthenticate(account?.personaId ?? 'faustin', remember, email.trim(), password);
      setStatus('success');
  ...
  const submitInvite = (event:FormEvent) => {
    ...
    window.setTimeout(() => {
      ...
      window.setTimeout(() => onAuthenticate('laetitia', true), 550);
    }, 550);
  };
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-main">
      ...
      <div className="auth-card">
        ...
        {screen === 'invite' && <>
          <button type="button" className="auth-back" onClick={() => switchScreen('login')}>← Retour à la connexion</button>
          <div className="auth-heading"><span className="auth-mode-chip">INVITATION DE DÉMONSTRATION</span><h2>Activez votre compte</h2><p>Compte invité : <b>Laetitia ATTOH</b><br />Rôle : Rondes & constats · périmètre RND-LET</p></div>
...
export default function Home() {
  ...
  const [personaId, setPersonaId] = useState<PersonaId>('faustin');
  ...
  const [fieldRequests, setFieldRequests] = useState<FieldRequest[]>([
    { id:'REQ-031', from:'Laetitia ATTOH', subject:'Infiltration légère · Atrium restaurant', note:'Photo ajoutée, origine à qualifier après la pluie.', status:'À traiter par Faustin' },
    { id:'REQ-030', from:'Sylvain DOUANE', subject:'WILO-01 · deuxième réarmement en 7 jours', note:'Service rétabli provisoirement, diagnostic demandé.', status:'À traiter par Faustin' },
  ]);
  ...
  const persistVendorReport = async (input:{ anomalyReference:string; vendorCode:string; file:File; reportType:'intervention_report'|'pv'|'quote'|'photo_bundle'; reportDate:string; summary:string; reserveNotes?:string; costAmount?:number }) => {
    if (session?.mode === 'demo') {
      ...
      flash(`Rapport simulé au nom de ${input.vendorCode} ; validation de Faustin requise.`);
      return;
    }
    ...
    try {
      ...
      flash(`${String(result.reference)} déposé au nom de ${input.vendorCode} ; validation de Faustin requise.`);
    } catch (error) {
  ...
  const signOut = async () => {
    ...
    window.localStorage.removeItem(SESSION_KEY); window.sessionStorage.removeItem(SESSION_KEY);
    setSignOutConfirm(false); setSession(null); setPasswordChangeRequirement(null); setPersonaId('faustin'); setView('workspace'); setToast(''); setDataState('demo'); setCanUploadVendorReport(false);
  };
  const resetDemo = () => {
    ...
    window.localStorage.removeItem(SESSION_KEY); window.sessionStorage.removeItem(SESSION_KEY);
    setSession(null); setPasswordChangeRequirement(null); setPersonaId('faustin'); setView('workspace'); setPreviousView('registry'); setAnomalies(seedAnomalies); setEquipmentItems(fallbackEquipment); setVendorReferences(fallbackVendors); setCanUploadVendorReport(false); setDataState('demo'); setReferenceCounts({ anomalies:seedAnomalies.length, equipment:fallbackEquipment.length, zones:0, profiles:0 }); setEscalations(seedEscalations);
    setFieldRequests([
      { id:'REQ-031', from:'Laetitia ATTOH', subject:'Infiltration légère · Atrium restaurant', note:'Photo ajoutée, origine à qualifier après la pluie.', status:'À traiter par Faustin' },
      { id:'REQ-030', from:'Sylvain DOUANE', subject:'WILO-01 · deuxième réarmement en 7 jours', note:'Service rétabli provisoirement, diagnostic demandé.', status:'À traiter par Faustin' },
    ]);
  ...
  const submitFieldRequest = (request: Omit<FieldRequest, 'id' | 'status'>) => {
    ...
    setFieldRequests((items) => [{ ...request, id, status:'À traiter par Faustin' }, ...items]);
    flash(`${id} transmise à Faustin — simulation locale.`);
  };
  const decideEscalation = (id:string, state:DecisionState, motive:string) => {
    ...
    flash(`${id} · décision ${state.toLowerCase()} et retour envoyé à Faustin.`);
  };
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="scope-box"><span>●</span><div><b>Immeuble BEHIRA</b><small>{session.mode === 'supabase' ? dataState === 'live' ? `${referenceCounts.anomalies} anomalies · ${referenceCounts.equipment} équipements · ${referenceCounts.zones} zones` : dataState === 'loading' ? `Synchronisation ${supabaseIntegration.environmentLabel}…` : 'Repli sur les données de démonstration' : 'Site principal · démonstration'}</small></div></div>
...
function DirectionWorkspace({ anomalies, equipment, escalations, onDecision, onOpen, onNavigate }: { anomalies:Anomaly[]; equipment:EquipmentItem[]; escalations:Escalation[]; onDecision:(id:string,state:DecisionState,motive:string)=>void; onOpen:(id:string)=>void; onNavigate:(view:View)=>void }) {
  ...
  const [newAgent, setNewAgent] = useState({name:'', email:'', role:'Agent terrain', scope:'WILO-01'});
...
function FaustinWorkspace({ escalations, fieldRequests, onEscalate, onOpen, onNavigate }: { escalations:Escalation[]; fieldRequests:FieldRequest[]; onEscalate:(request:FieldRequest)=>void; onOpen:(id:string)=>void; onNavigate:(view:View)=>void }) {
  const responses = escalations.filter((item) => item.state !== 'À décider');
  return <>
    <WorkspaceIntro kicker="FACILITY MANAGER" title="Bonjour Faustin" description="Qualifiez, affectez, relancez et préparez les arbitrages Direction." badge="Pilotage opérationnel" />
    <AnswerStrip todo="2 qualifications" risk="RIA-01 critique" due="ASC-A2 en retard" proof="2 dossiers" />
    <section className="faustin-personal-grid"><article className="panel"><div className="panel-head"><div><h3>Remontées terrain</h3><p>Demandes reçues des agents et de Laetitia</p></div><Badge tone="orange">{fieldRequests.filter((item) => item.status === 'À traiter par Faustin').length} à traiter</Badge></div><div className="field-request-list">{fieldRequests.map((request) => {
      const exceedsDelegation = /deuxième|critique|coût|sécurité/i.test(`${request.subject} ${request.note}`);
      return <article key={request.id}><div><Badge tone={request.status === 'Transmise à Direction' ? 'blue' : 'orange'}>{request.status}</Badge><span>{request.id} · {request.from}</span>{exceedsDelegation && request.status === 'À traiter par Faustin' && <Badge tone="critical">Hors délégation</Badge>}</div><h4>{request.subject}</h4><p>{request.note}</p>{request.status === 'À traiter par Faustin' ? <div><button className="primary-button qualify-action" onClick={() => onNavigate('manager')}>Qualifier maintenant</button><button className={`escalate-action ${exceedsDelegation ? 'is-recommended' : ''}`} onClick={() => onEscalate(request)}>Soumettre à l’Administration</button></div> : <small>En attente de décision de l’Administration.</small>}</article>;
...
const agentTaskSets: Record<'evariste'|'sylvain', AgentTask[]> = {
  evariste:[
    { id:'ACT-081', asset:'GE-01', title:'Ronde démarrage et mode AUTO', due:'Aujourd’hui · 10:00', risk:'Continuité électrique', status:'À faire', proof:false, detail:'Relever tension batterie, niveau carburant et confirmer le mode AUTO.' },
    { id:'ACT-079', asset:'ASC-A2', title:'Relever le code après arrêt R+7', due:'En retard · 23 août', risk:'Perte de redondance', status:'En cours', proof:false, delayed:true, detail:'Photographier le code défaut et confirmer le fonctionnement de l’interphone.' },
    { id:'ACT-076', asset:'ASC-A1', title:'Essai éclairage de secours', due:'Terminé · 08:15', risk:'Aucun après essai', status:'Terminé', proof:true, detail:'Essai concluant, photo et valeur de tension jointes.' },
  ],
  sylvain:[
    { id:'ACT-088', asset:'RIA-01', title:'Contrôler pression et coffret GMP', due:'Aujourd’hui · 10:30', risk:'Sécurité incendie critique', status:'À faire', proof:false, detail:'Relever pression, position des vannes et état ON/OFF du coffret.' },
    { id:'ACT-084', asset:'WILO-01', title:'Diagnostiquer défaut pompe P1', due:'Aujourd’hui · 12:00', risk:'Perte de redondance P1/P2', status:'En cours', proof:false, detail:'Deuxième défaut en sept jours. Le réarmement ne vaut pas clôture.' },
    { id:'ACT-080', asset:'WILO-01', title:'Contrôle fuite collecteur', due:'Terminé · 08:05', risk:'Fuite contenue', status:'Terminé', proof:true, detail:'Suintement contenu et photo transmise à Faustin.' },
  ],
};
...
function AgentWorkspace({ persona, anomalies, vendors, canUploadVendorReport, vendorReportBusy, onVendorReport, onFieldRequest, flash }: { persona:Persona; anomalies:Anomaly[]; vendors:OperationalVendor[]; canUploadVendorReport:boolean; vendorReportBusy:boolean; onVendorReport:(input:VendorReportInput)=>Promise<void>; onFieldRequest:(request:Omit<FieldRequest,'id'|'status'>)=>void; flash:(message:string)=>void }) {
  const agentKey = persona.id as 'evariste'|'sylvain';
...
function InternalVendorReportPanel({ anomalies, vendors, canUpload, busy, onSubmit }: { anomalies:Anomaly[]; vendors:OperationalVendor[]; canUpload:boolean; busy:boolean; onSubmit:(input:VendorReportInput)=>Promise<void> }) {
  ...
  return <section className={`panel internal-vendor-report ${canUpload ? 'is-authorized' : ''}`}>
    ...
    <div className="internal-access-rule"><span>⌁</span><div><b>Aucun accès direct pour les prestataires</b><p>Un agent interne autorisé rattache le rapport, le fichier et les métadonnées au dossier. Faustin contrôle ensuite la preuve.</p></div></div>
    {!canUpload ? <div className="permission-empty"><p>Ce profil ne dispose pas du droit nominatif de dépôt. Évariste et Sylvain sont les seuls agents internes habilités.</p><button className="secondary-button" type="button" disabled>Déposer un rapport prestataire</button></div> :
    <form className="internal-vendor-form" onSubmit={submit}>
      ...
      <button className="primary-button" disabled={busy || localBusy || !file || !anomalyReference || !vendorCode || !summary.trim()}>{busy || localBusy ? 'Dépôt en cours…' : 'Déposer pour validation de Faustin'}</button>
    </form>}
  </section>;
}
...
function LaetitiaWorkspace({ fieldRequests, onFieldRequest, flash }: { fieldRequests:FieldRequest[]; onFieldRequest:(request:Omit<FieldRequest,'id'|'status'>)=>void; flash:(message:string)=>void }) {
  ...
  const submit = (event:FormEvent) => {
    ...
    onFieldRequest({ from:'Laetitia ATTOH', subject:`${category} · ${zone}`, note:`${title.trim()}${photo ? ' · photo de zone jointe' : ' · photo à compléter'}` });
  ...
  return <>
    ...
    {(dashboardTab === 'overview' || dashboardTab === 'actions') && <section id={`dashboard-panel-${dashboardTab}`} role="tabpanel" aria-labelledby={`dashboard-tab-${dashboardTab}`} className={`direction-grid dashboard-tab-panel ${dashboardTab === 'actions' ? 'actions-view' : ''}`} aria-label="Tableau de bord en cinq angles de décision">
      <article className="panel direction-block todo-block">
        ...
        <button className="text-action" onClick={() => onNavigate(readOnly ? 'registry' : 'manager')}>{readOnly ? 'Consulter le registre →' : 'Ouvrir la file de Faustin →'}</button>
      </article>
      ...
      <article className="panel direction-block risks-block">
        ...
        <div className="risk-focus"><span>!</span><div><b>RIA-01 sous surveillance</b><small>Pression incendie instable</small></div></div>
      ...
      <article className="panel direction-block decisions-block">
        ...
        <div className="decision-list">
          <button onClick={() => onOpen('ANO-0241','dashboard')}><span className="decision-priority critical">1</span><div><b>Qualifier RIA-01 en priorité critique</b><small>Sécurité incendie · décision requise avant 10:30</small></div><span>{readOnly ? 'Consulter →' : 'Décider →'}</span></button>
          <button onClick={() => onOpen('ANO-0238','dashboard')}><span className="decision-priority warning">2</span><div><b>Relancer le prestataire ASC-A2</b><small>Intervention en retard · impact usagers</small></div><span>{readOnly ? 'Consulter →' : 'Relancer →'}</span></button>
...
function Detail({ anomaly, onBack, onStatus, onProof, onVerify, offlineSync, persistenceEnabled, readOnly = false, canVerify = false, busy = false }: { anomaly:Anomaly; onBack:()=>void; onStatus:(s:Status)=>void; onProof:(file:File)=>void; onVerify:()=>void; offlineSync:ReturnType<typeof useOfflineSync>; persistenceEnabled:boolean; readOnly?:boolean; canVerify?:boolean; busy?:boolean }) {
  ...
  const estimatedCost = anomaly.asset === 'WILO-01' ? 280000 : anomaly.asset === 'GE-01' ? 3800000 : anomaly.asset === 'ASC-A2' ? 1650000 : 0;
  const overThreshold = estimatedCost >= 350000;
  return <>

...
const checks = [
  ...
  ['Montant du dossier raccordé à l’arbitrage existant', page.includes('decision={escalations.find') && page.includes('snapshot.costs.map(mapOperationalCostDecision)') && !page.includes("const estimatedCost = anomaly.asset") && page.includes('Pièce financière non reliée') && !page.includes('DEVIS-INTERVENTION.pdf')],
  ['Seuil financier unique et non modifiable silencieusement', page.includes('const DECISION_THRESHOLD_FCFA = 400_000') && page.includes('setDecisionThreshold(snapshot.financialDecisionParameter.value)') && page.includes('thresholdAmount:item.thresholdAmount') && !page.includes('setThreshold(Number')],
  ...
  ['Administration cliente neutralisée dans le miroir', accessWorkspace.includes('ne modifie les comptes, les droits ou les utilisateurs réels') && accessWorkspace.includes('Simulation locale') && accessWorkspace.includes("users.filter((user) => user.id !== 'administration')") && accessWorkspace.includes("audience === 'administration' && <option>Administration</option>") && !accessWorkspace.includes('service_role') && !accessWorkspace.includes('createUser(') && !accessWorkspace.includes('deleteUser(')],
  ...
  ['Synthèse anti-dossier-zombie prioritaire dans le dossier central', page.includes('<div className="dossier-continuity"><AntiZombieSummary data={resolveAntiZombieSummary(anomaly)} variant="detailed" /></div>') && page.indexOf('className="dossier-continuity"') < page.indexOf('className="dossier-tabs"') && css.includes('.dossier-continuity .anti-zombie-summary{margin:0 0 14px;box-shadow:none}')],
  ['Onglets du dossier accessibles', page.includes('role="tablist"') && ['overview','finance','evidence','history'].every((section) => page.includes(`aria-selected={section === '${section}'}`)) && ['dossier-overview-panel','dossier-finance-panel','dossier-evidence-panel','dossier-history-panel'].every((id) => page.includes(`id="${id}" role="tabpanel"`))],
  ['Historique du dossier canonique avec repli honnête', page.includes('const historyEvents = anomaly.history ?? []') && page.includes('Événements métier du dossier') && page.includes('formatHistoryMoment(event.occurredAt)') && page.includes('Historique métier indisponible') && page.includes('Les repères ci-dessous ne remplacent pas un journal métier') && !page.includes('Étape validée et historisée') && !page.includes('Historique complet')],
  ...
  ['Refus de preuve explicite dans la file agent', page.includes('className="task-proof-rejection" role="alert"') && page.includes('Refusée · remplacement requis') && css.includes('.task-proof-rejection{') && css.includes('border:1px solid var(--danger-border)') && css.includes('background:var(--danger-surface)') && css.includes('color:var(--danger-text)')],
  ...
  ['Destinations Plus sans titre jumeau', equipmentWorkspace.includes('className="visually-hidden">Équipements') && costsWorkspace.includes('className="visually-hidden">Coûts documentés') && accessWorkspace.includes('className="visually-hidden">Utilisateurs et droits') && parametersWorkspace.includes('className="visually-hidden">Seuils et paramètres') && finalDeclaration('.equipment-workspace-hero', 'background') === 'transparent' && finalDeclaration('.access-action-tabs button.active', 'background') === 'var(--teal)'],
  ...
  ['C11-A cockpit sans score bâtiment inventé', cockpit.includes('Score non calculable — données insuffisantes') && cockpit.includes("dataState: HealthDataState") && cockpit.includes('building={null}') && !cockpit.includes('82/100')],
  ['C11-A état des données explicite', ['DÉMO · NON CALCULABLE','PARTIELLE · CHARGEMENT','PARTIELLE · NON CALCULABLE'].every((label) => cockpit.includes(label)) && ['criticité des équipements validée','règles et pondérations approuvées','couverture des mesures, maintenances et anomalies','historique d’instantanés datés'].every((item) => cockpit.includes(item))],
  ['C11-A scores agents non inventés', page.includes('aria-label="Scores des agents non calculables"') && page.includes('<small>Score non calculable</small>') && page.includes('<strong>—</strong>')],
  ['C11-A score WILO suspendu', page.includes('Le calcul WILO-01 reste suspendu jusqu’à la validation des huit décisions métier') && !page.includes('78/100') && !page.includes('Pression sous le seuil <b>-8</b>') && !page.includes('Défaut P1 récurrent <b>-10</b>')],
  ['C11-A notification score suspendue', notificationCenter.includes("id: 'health'") && notificationCenter.includes('Suspendu : score non calculable tant que les règles métier ne sont pas validées') && notificationCenter.includes("const scoreRuleSuspended = item.id === 'health'")],
  ['C11-A KPI santé lisibles sur mobile', css.includes('.health-score-panel .health-kpi-grid{grid-template-columns:1fr 1fr}') && css.includes('.health-score-panel .health-kpi-grid{grid-template-columns:1fr}')],
]

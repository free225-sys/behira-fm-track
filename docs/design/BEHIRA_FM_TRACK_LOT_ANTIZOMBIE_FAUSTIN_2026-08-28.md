# BEHIRA FM Track — Lot `AntiZombieSummary` Faustin
...
Un composant partagé et réutilisable `AntiZombieSummary` a été créé puis intégré une seule fois dans le cockpit Facility Manager de Faustin. L’ancien bandeau incomplet à quatre champs a été remplacé dans ce seul écran.
...
- `app/page.tsx` : adaptateur des données Faustin et intégration unique.
...
- `outputs/design/anti-zombie-faustin-desktop.png` : capture desktop locale.
- `outputs/design/anti-zombie-faustin-mobile.png` : capture mobile locale.

## 3. Contrat de données

Le composant reçoit un objet `AntiZombieSummaryData`. Il n’évalue aucun seuil, ne calcule aucun retard et n’accorde aucune action. L’adaptateur Faustin lui transmet les faits déjà disponibles ; le normaliseur applique seulement les libellés de repli requis.
...
| Prochaine action | File Faustin active `tab` et libellés déjà présents dans l’ancien bandeau | `proof` affiche « Obtenir la preuve » ; les autres files conservent « Confirmer le diagnostic ». Aucun nouveau moteur de workflow. |
...
| Preuve attendue | Définition existante du dossier WILO | WILO-01 conserve « Photo du manomètre et rapport d’intervention ». Les autres équipements affichent « Preuve attendue non définie » au lieu d’une valeur inventée. |
...
- matrice de preuve attendue pour les équipements autres que WILO-01 ;
...
- Desktop : `outputs/design/anti-zombie-faustin-desktop.png`
- Mobile : `outputs/design/anti-zombie-faustin-mobile.png`
...
| Intégration unique dans Faustin | Réussi |
| Absence dans Évariste / autres personas | Réussi dans le navigateur et par contrôle statique |
...
- Les preuves attendues ne sont explicitement définies que pour WILO-01 dans l’interface existante.

...
Le prototype contient déjà les parcours et le vocabulaire métier structurants : espaces par rôle, centre de décision de Faustin, dossier complet, registre, rondes, scores, gestion des utilisateurs et authentification. Il ne faut pas les reconstruire.
...
La bonne stratégie n’est donc pas une refonte globale. Il faut d’abord consolider les tokens et les primitives partagées, puis recalibrer un seul écran pilote : le **Centre de décision Faustin**. Après validation, les autres familles d’écrans pourront migrer progressivement.
...
- La feuille `app/globals.css` contient le design historique, les ajouts successifs, l’authentification, les personas, les maquettes Wilo, Faustin, dossier et Administration.
...
| Centre de décision Faustin | Facility Manager | Maquette interactive riche | `app/page.tsx:1167` |
| Espace agent technique | Évariste / Sylvain | Présent | `app/page.tsx:1016` |
| Espace Laetitia | Laetitia | Présent, double mission visible | `app/page.tsx:1084` |
| Tableau de bord consolidé | Administration / Faustin | Présent en cinq blocs | `app/page.tsx:1110` |
| Registre des anomalies | Administration / Faustin | Présent, filtres et mobile | `app/page.tsx:1159` |
| Dossier complet | Selon le rôle | Présent, quatre sections | `app/page.tsx:1197` |
| Ronde Wilo | Sylvain / Faustin | Maquette interactive hors ligne | `app/page.tsx:1230` |
| Ronde rapide terrain | Évariste / Laetitia | Présente | `app/page.tsx:1241` |
| Rapport prestataire déposé en interne | Évariste / Sylvain | Présent et protégé par permission | `app/page.tsx:1046` |
...
- La distinction entre Administration, Facility Manager, agents techniques et Laetitia.
...
- Le seuil d’approbation de 350 000 FCFA et son explication.
- Le verrou de clôture critique sans preuve acceptée.
- La saisie directe des rondes, sans import de reporting.
- Le fonctionnement hors ligne présenté dans le module Wilo.
...
La feuille utilise plus de vingt formes de rayon et de nombreuses ombres différentes. Les cartes sont globalement sobres, mais leur géométrie varie selon leur date d’ajout. Plusieurs héros utilisent encore un dégradé sombre, notamment le centre Faustin et Wilo : `app/globals.css:192` et `app/globals.css:185`.
...
Le registre et le centre Faustin approchent déjà la densité Dub attendue. Le tableau de bord Administration reste davantage composé de grandes cartes, tandis que les écrans agents utilisent encore plusieurs blocs de synthèse. Les informations prioritaires ne sont pas toujours placées au même endroit.
...
### 5.8 Vocabulaire Administration / Direction / Frédéric — priorité moyenne

Le code distingue correctement le rôle et son titulaire, mais l’interface alterne encore entre « Direction », « Administration » et « Frédéric » : `app/page.tsx:118`, `app/page.tsx:858`, `app/page.tsx:959`.

**Décision proposée :** utiliser « Administration » pour la capacité métier, et réserver « Frédéric AMANY » à l’identité de l’utilisateur connecté.
...
5. Valider si le centre Faustin devient bien l’écran pilote avant migration des autres écrans.
...
### Lot UI-1 — Écran pilote : Centre de décision Faustin
...
### Lot UI-4 — Rondes Wilo, agents et Laetitia

**Objectif :** harmoniser les formulaires terrain et les états hors ligne.

**Fichiers concernés :** `Report`, `AgentWorkspace`, `LaetitiaWorkspace`, styles Wilo et formulaires.
...
**Critères d’acceptation :** aucune importation, saisie directe explicite, actions tactiles, états hors ligne visibles, double mission Laetitia préservée.
...
- Centre de décision Faustin comme écran pilote ;
